...
| 版本     | 核心主题             | 主要产出                                                              | 状态      |
| -------- | -------------------- | --------------------------------------------------------------------- | --------- |
| `v2.4.0` | 多供应商上游适配     | LiteLLM 接入、上游适配层、非 OpenAI 上游转发能力                      | `Planned` |
| `v2.5.0` | 结构化代理日志与并发 | `trace` 体系、代理日志页、单模型并发处理                              | `Planned` |
| `v2.6.0` | 模型路由重构         | `published_model / target / failover_pool` 配置模型、动态路由、热切换 | `Planned` |
...
4. 后端：把现有运行中配置应用能力扩展到整套路由配置，保存后自动热切换。
