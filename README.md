...
> - 可以在设置面板中开启“开机自启动”功能（仅打包版支持，开发模式下不可用）
> - **系统托盘**：程序启动后会显示在系统托盘。左键单击托盘图标可显示/隐藏窗口；右键单击可打开功能菜单（包含退出选项）。点击窗口关闭按钮默认最小化到托盘。

> [!WARNING]
> - 需要管理员权限
> - macOS 端如提示“包已损坏”，请参考 [macOS 解决 “包已损坏” 问题](#macos-解决-包已损坏-问题)
> - 如 trae 端添加模型失败，请参考 [trae 端提示 “添加模型失败” 的排查方案](#trae-端提示-添加模型失败-的排查方案)
> - 如果检查更新时遇到 "GitHub 返回 403"，请在 `.env` 中配置 `GITHUB_TOKEN` 以提高 API 限额（详见开发指南）。
